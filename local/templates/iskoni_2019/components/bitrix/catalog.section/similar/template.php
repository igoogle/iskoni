...
?>
<? if ($arResult["ITEMS"]): ?>
<div class="houses__look section">
  <div class="container">
    <div class="section-head">
      <h2 class="section-title text-left">Похожие проекты</h2>
    </div>
  </div>
      ...
      <div class="card">
        <div class="card-thumb">
          <a href="<?=$arItem["DETAIL_PAGE_URL"]?>">
            <?php
                        ...
                        ?>
            <img class="lazy" src="/bitrix/images/1.gif" data-src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>"
        ...
        <div class="card-body">
          ...
          <div class="card-footer">
            ...
            <a class="btn btn-orange" href="<?=$arItem["DETAIL_PAGE_URL"]?>">Перпейти</a>
...
<?
	$arSelect = Array("ID", "NAME", "PROPERTY_FILTER_DETAIL_TEXT");
	$arFilter = Array("IBLOCK_ID"=>9, "ACTIVE"=>"Y","NAME" => $APPLICATION->GetCurPage());
	$element = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect) -> GetNext();
	if ($element['PROPERTY_FILTER_DETAIL_TEXT_VALUE']) {
		if (!isset($_GET["PAGEN_1"])) {
			echo($element['~PROPERTY_FILTER_DETAIL_TEXT_VALUE']['TEXT']);
		}
	} else {
		if (!strpos($_SERVER["REQUEST_URI"] , "/filter/") && !isset($_GET["PAGEN_1"])) {
...
?>
<div
  class="bx_catalog_list_home col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>">
  ...
  <div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>">
    <div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
      <a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
        class="bx_catalog_item_images" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>')"
        title="<? echo $imgTitle; ?>">
        <?
      ...
      <div class="bx_catalog_item_controls">
        ...
        <div class="bx_catalog_item_controls_blockone">
          <div style="display: inline-block;position: relative;">
            ...
            <input type="text" class="bx_col_input" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["
              PRODUCT_QUANTITY_VARIABLE"]; ?>" value="
            <? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
            <a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="bx_bt_button_type_2 bx_small"
              rel="nofollow">+</a>
        ...
        <div class="bx_catalog_item_controls_blocktwo">
          ...
          <?
				...
				{
				?>
          <a id="<? echo $arItemIDs['SUBSCRIBE_LINK']; ?>" class="bx_bt_button_type_2 bx_medium"
            href="javascript:void(0)">
            <?
            ...
            <td>
              <?
								...
								)
								{
									...
									{
										?><label><input type="radio" name="<? echo $arParams['PRODUCT_PROPS_VARIABLE']; ?>[<? echo $propID; ?>]"
                  value="<? echo $valueID; ?>" <? echo ($valueID==$propInfo['SELECTED'] ? '"checked"' : '' ); ?>>
                <? echo $value; ?>
              </label><br>
              <?
									}
	...
	else
	{
      ...
      <div class="bx_catalog_item_controls no_touch">
        <?
			...
			?>
        <div class="bx_catalog_item_controls_blockone">
          <a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="bx_bt_button_type_2 bx_small"
            rel="nofollow">-</a>
          <input type="text" class="bx_col_input" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["
            PRODUCT_QUANTITY_VARIABLE"]; ?>" value="
          <? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
...
<script type="text/javascript">
BX.message({
  BTN_MESSAGE_BASKET_REDIRECT: '<? echo GetMessageJS('
  CT_BCS_CATALOG_BTN_MESSAGE_BASKET_REDIRECT '); ?>',
  BASKET_URL: '<? echo $arParams["BASKET_URL"]; ?>',
  ADD_TO_BASKET_OK: '<? echo GetMessageJS('
  ADD_TO_BASKET_OK '); ?>',
  TITLE_ERROR: '<? echo GetMessageJS('
  CT_BCS_CATALOG_TITLE_ERROR ') ?>',
  TITLE_BASKET_PROPS: '<? echo GetMessageJS('
  CT_BCS_CATALOG_TITLE_BASKET_PROPS ') ?>',
  TITLE_SUCCESSFUL: '<? echo GetMessageJS('
  ADD_TO_BASKET_OK '); ?>',
  BASKET_UNKNOWN_ERROR: '<? echo GetMessageJS('
  CT_BCS_CATALOG_BASKET_UNKNOWN_ERROR ') ?>',
  BTN_MESSAGE_SEND_PROPS: '<? echo GetMessageJS('
  CT_BCS_CATALOG_BTN_MESSAGE_SEND_PROPS '); ?>',
  BTN_MESSAGE_CLOSE: '<? echo GetMessageJS('
  CT_BCS_CATALOG_BTN_MESSAGE_CLOSE ') ?>',
  BTN_MESSAGE_CLOSE_POPUP: '<? echo GetMessageJS('
  CT_BCS_CATALOG_BTN_MESSAGE_CLOSE_POPUP '); ?>',
  COMPARE_MESSAGE_OK: '<? echo GetMessageJS('
  CT_BCS_CATALOG_MESS_COMPARE_OK ') ?>',
  COMPARE_UNKNOWN_ERROR: '<? echo GetMessageJS('
  CT_BCS_CATALOG_MESS_COMPARE_UNKNOWN_ERROR ') ?>',
  COMPARE_TITLE: '<? echo GetMessageJS('
  CT_BCS_CATALOG_MESS_COMPARE_TITLE ') ?>',
  BTN_MESSAGE_COMPARE_REDIRECT: '<? echo GetMessageJS('
